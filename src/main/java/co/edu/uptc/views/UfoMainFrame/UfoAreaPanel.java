...
public class UfoAreaPanel extends JPanel {
    ...
    public Ufo selectUfoAtPosition(int x, int y) {
        gamePanel.getUfoMainView().getPresenter().sendXPosition(x);
        gamePanel.getUfoMainView().getPresenter().sendYPosition(y);
        Ufo ufo = gamePanel.getUfoMainView().getPresenter().selectUfoAtPosition();
        if (ufo != null) {
            selectedUfo = ufo;
        }
        repaint();
    ...
    private void changeSelectedUfoSpeed(int delta) {
        gamePanel.getUfoMainView().getPresenter().sendSelectedUfoSpeed(delta);
        gamePanel.getUfoMainView().getPresenter().changeSelectedUfoSpeed();
    }
